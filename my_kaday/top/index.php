...
//求人の表示
$stmt_job_post = $pdo->prepare("SELECT * FROM job_post WHERE life_flg= :life_flg");
...
if($status_job_post==false){
  ...
  while( $result_job_post = $stmt_job_post->fetch(PDO::FETCH_ASSOC)){
    ...
    $view_job_post .= $result_job_post["job_title"];
    $view_job_post .='</div>';
    $view_job_post .='<div class="col-xs-2">';
    $view_job_post .='<a href="../job_post/job_post_view.php?job_post_id='.$result_job_post["id"].'" class="btn btn-info btn-sm" target="_blank">詳細</a>';
    $view_job_post .='</div>';
    $view_job_post .='</div>';
    $view_job_post .='</li>';
  }
...
if($status_interview_date_fix==false){
  ...
  while( $result_interview_date_fix = $stmt_interview_date_fix->fetch(PDO::FETCH_ASSOC)){
    ...
    $view_interview_date_fix .= $result_interview_date_fix["interviewee_name"].'<br>'.$interview_type_str;
    $view_interview_date_fix .='</div>';
    $view_interview_date_fix .='<div class="col-xs-2">';
    $view_interview_date_fix .='<a href="../setting/interview_confirm01.php?interview_id='.$result_interview_date_fix["id"].'" class="btn btn-warning btn-sm">確定</a>';
    $view_interview_date_fix .='</div>';
    $view_interview_date_fix .='</div>';
...
if($status_new_interviewee==false){
  ...
  while( $result_new_interviewee = $stmt_new_interviewee->fetch(PDO::FETCH_ASSOC)){
    ...
    $interview_time = remove_second($result_new_interviewee["indate"]);
    $view_new_interviewee .= $result_new_interviewee["interviewee_name"].'/応募日時:'.$interview_time.'<br>'.$result_new_interviewee["job_title"];
    $view_new_interviewee .='</div>';
    $view_new_interviewee .='<div class="col-xs-2">';
    $view_new_interviewee .='<a href="../setting/interviewee_detail.php?target_interviewee_id='.$result_new_interviewee["id"].'" class="btn btn-info btn-sm">詳細</a>';
    $view_new_interviewee .='</div>';
    $view_new_interviewee .='</div>';
...
if($status_s_or_f==false){
  ...
  while( $result_s_or_f = $stmt_s_or_f->fetch(PDO::FETCH_ASSOC)){
    ...
    $view_s_or_f .= $result_s_or_f["interviewee_name"].'<br>'.$interview_type_str.' '.$interview_date;
    $view_s_or_f .='</div>';
    $view_s_or_f .='<div class="col-xs-2">';
    $view_s_or_f .='<a href="../result/output_data.php?interview_id='.$result_s_or_f["id"].'" class="btn btn-warning btn-sm">入力</a>';
    $view_s_or_f .='</div>';
    $view_s_or_f .='</div>';
    $view_s_or_f .='</li>';
  }
...
if($status_comming_interview==false){
  ...
  while( $result_comming_interview = $stmt_comming_interview->fetch(PDO::FETCH_ASSOC)){
    ...
    $view_comming_interview .= $result_comming_interview["interviewee_name"].'<br>'.$interview_type_str.' '.$interview_time;
    $view_comming_interview .='</div>';
    $view_comming_interview .='<div class="col-xs-2">';
    $view_comming_interview .='<a href="../result/input_data.php?interview_id='.$result_comming_interview["id"].'" class="btn btn-info btn-sm">詳細</a>';
    $view_comming_interview .='</div>';
    $view_comming_interview .='</div>';
...
$stmt_interview_date_setting = $pdo->prepare("select interviewee_info.interviewee_name,interview.id,interview.interviewee_id,interview.stage_flg from interviewee_info LEFT OUTER JOIN interview ON interviewee_info.id = interview.interviewee_id WHERE interview.stage_flg = :stage_flg OR interview.stage_flg IS NULL");
...
if($status_interview_date_setting==false){
  ...
  while( $result_interview_date_setting = $stmt_interview_date_setting->fetch(PDO::FETCH_ASSOC)){
    ...
    $view_interview_date_setting .= $result_interview_date_setting["interviewee_name"];
    $view_interview_date_setting .='</div>';
    $view_interview_date_setting .='<div class="col-xs-2">';
    if($result_interview_date_setting["stage_flg"] ==6){
      $view_interview_date_setting .='<a href="../setting/interview_resetting.php?interview_id='.$result_interview_date_setting["id"].'" class="btn btn-warning btn-sm">再調整</a>';
    }elseif($result_interview_date_setting["stage_flg"] == NULL){
      $view_interview_date_setting .='<a href="../setting/interview01_setting.php?interview_type_num=1&target_interviewee_id='.$result_interview_date_setting["interviewee_id"].'" class="btn btn-warning btn-sm">調整</a>';
    }
    $view_interview_date_setting .='</div>';
    $view_interview_date_setting .='</div>';
...
if($status_evalueation_input==false){
  ...
  while( $result_evalueation_input = $stmt_evalueation_input->fetch(PDO::FETCH_ASSOC)){
    ...
    $view_evalueation_input .= $result_evalueation_input["interviewee_name"].'<br>'.$interview_type_str;
    $view_evalueation_input .='</div>';
    $view_evalueation_input .='<div class="col-xs-2">';
    $view_evalueation_input .='<a href="../result/input_data.php?interview_id='.$result_evalueation_input["id"].'" class="btn btn-warning btn-sm">入力</a>';
    $view_evalueation_input .='</div>';
    $view_evalueation_input .='</div>';
...
<div class="container-fluid">
  <div class="row">
    <h2 class="text-center">タスク・新着情報</h2>
  </div>
  <!-- ここから管理領域 -->
