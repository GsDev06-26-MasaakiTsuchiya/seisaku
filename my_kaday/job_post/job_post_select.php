...
if($status==false){
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    if($result["life_flg"]==1){
      $view .=' <button class="openClose btn btn-warning btn-sm open" id="'.$result["id"].'">休止中</button>';
    }else{
      $view .=' <button class="openClose btn-success btn btn-sm close" id="'.$result["id"].'">掲載中</button>';
    }
    $view .= '</td>';
    $view .= '</tr>';
